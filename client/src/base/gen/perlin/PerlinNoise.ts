...
export class PerlinNoise {
  ...
  public static create (): PerlinNoise {
  ...
  public withSeed (seed: number): PerlinNoise {
    if (seed > 0 && seed < 1) {
      // Scale the seed out
      seed *= 65536;
    }
  ...
  // 2D simplex noise
  public generateSimplex2 (xin: number, yin: number): number {
  ...
  // 3D simplex noise
  public generateSimplex3 (xin: number, yin: number, zin: number): number {
  ...
  // 2D Perlin Noise
  public generatePerlin2 (x: number, y: number): number {
    ...
    return PerlinNoise.lerp(
        PerlinNoise.lerp(n00, n10, u),
        PerlinNoise.lerp(n01, n11, u),
        PerlinNoise.fade(y));
  };

  // 3D Perlin Noise
  public generatePerlin3 (x: number, y: number, z: number) {
    // Find unit grid cell containing point
    ...
    return PerlinNoise.lerp(
        PerlinNoise.lerp(
            PerlinNoise.lerp(n000, n100, u),
            PerlinNoise.lerp(n001, n101, u), w),
        PerlinNoise.lerp(
            PerlinNoise.lerp(n010, n110, u),
            PerlinNoise.lerp(n011, n111, u), w),
        v);
  }
